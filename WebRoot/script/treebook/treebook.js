	...
	function adds(rual){
	    $("pwid_id").value=$("wid_id").value;
	 	$('wordbookform').ajaxSubmit(lang.rurl+"/treeadd.nk");
	}
	//-----------修改-------------------------
	function updates(){
		$('wordbookform').ajaxSubmit(lang.rurl+"/treeupdate.nk");
	}
		...
		if(ischeck){
			var r=confirm(lang.L1);
	  		if(r==true){
				...
				dataform.action=lang.rurl+'/treedel.nk';
	...
	function onsuccess(a){
		...
		case '1':
			...
			setTimeout(function(){
				if($('wid_id').value=='-1')
					$('wid_id').value='1';
				try{$('treewod').LoadData($('trepwid_id').value);}catch(e){}//刷新树形菜单
				try{$('treewod').LoadData($('wid_id').value);}catch(e){}//刷新树形菜单
				setTimeout(function(){
					try{$('treewod').opennode($('wid_id').value);}catch(e){};// 打开本级
	            }, 500);
            }, 500);
			break;
		case '2':
		...
		case '5':
			...
			setTimeout(function(){
				...
				try{$('treewod').LoadData($('trepwid_id').value);}catch(e){}//刷新树形菜单
				try{$('treewod').LoadData($('wid_id').value);}catch(e){}//刷新树形菜单
				try{$('treewod').opennode($('trepwid_id').value);}catch(e){};// 打开本级
            }, 500);
			break;
		case '6':
	...
	function tree_titileclick(node){
		$('wtext_id').setValue(node.ttext,true);
		$('wcompositor_id').setValue(node.tpx,true);
